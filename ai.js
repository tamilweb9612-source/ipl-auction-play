...
const bowlingLogic = {
  getBowlerType: (player) => {
    const role =
      player.roleKey?.toLowerCase() || player.role?.toLowerCase() || "";
    const name = (player.name || "").toLowerCase();
    let type = "pacer";

    if (
      role.includes("off") ||
      name.includes("ashwin") ||
      name.includes("narine")
    )
      type = "off-spinner";
    else if (
      role.includes("leg") ||
      name.includes("chahal") ||
      name.includes("rashid")
    )
      type = "leg-spinner";
  ...
  prepareBowlersForInnings: (playing11) => {
    ...
    // This is critical to prevent stats from bleeding across matches
    ...
    // Ensure 5+ bowlers if shortage
    if (bowlers.length < 5) {
      const others = playing11.filter(
        (p) => !bowlers.find((b) => b.name === p.name),
      );
      if (others.length > 0) {
        bowlers.push({
          ...others[0],
    ...
    // Sort by skill
    const ranked = bowlers.sort(
      (a, b) => (b.stats?.bowl || 0) - (a.stats?.bowl || 0),
    );

    // Refine Overs for Part Timers
    ranked.forEach((b) => {
      if (b.isPartTime) {
        b.maxOvers = 2;
        b.remaining = 2;
    ...
    // Adjust if 6+ bowlers (distribute overs)
    if (ranked.length >= 6) {
      if (ranked[4]) {
        ranked[4].maxOvers = 2;
        ranked[4].remaining = 2;
      }
      if (ranked[5]) {
        ranked[5].maxOvers = 2;
        ranked[5].remaining = 2;
      }
    }
...
const battingLogic = {
  getRole: (p) => {
    ...
    if (n.includes("russell") || n.includes("sky") || n.includes("maxwell"))
      return "finisher";
  ...
  getWicketType: (batter, bowler, phase) => {
    ...
    if (phase === "death" && r < 0.1) return "run out";
    ...
    if (bowlingLogic.getBowlerType(bowler).includes("spin")) {
      if (wR < 0.5) return "caught";
      if (wR < 0.75) return "lbw";
      if (wR < 0.9) return "bowled";
      return "stumped";
    } else {
      // Pacers
      if (wR < 0.6) return "caught";
  ...
  calculateBallOutcome: (batter, bowler, matchState, venue) => {
    ...
    if (diff > 15) {
      weights[4] += 5;
      weights[6] += 3;
      weights["W"] = Math.max(1, weights["W"] - 4);
    } else if (diff < -15) {
      weights[0] += 6;
      weights["W"] += 5;
      weights[6] -= 2;
...
function runFullTournament(tourneyTeams) {
  ...
  const getPStat = (name) => {
    if (!allStats[name])
      allStats[name] = { name, runs: 0, wkts: 0, pts: 0, fours: 0, sixes: 0 };
  ...
  tourneyTeams.forEach((t) => {
    t.stats.nrr = (
      t.stats.rs / Math.max(1, t.stats.of) -
      t.stats.rc / Math.max(1, t.stats.ob)
    ).toFixed(3);
  ...
  if (tourneyTeams.length >= 4) {
    ...
    const lQ1 =
      q1.winnerName === tourneyTeams[0].name
        ? tourneyTeams[1]
        : tourneyTeams[0];
    const wElim =
      elim.winnerName === tourneyTeams[2].name
        ? tourneyTeams[2]
        : tourneyTeams[3];
  ...
  const bestEco = statsArr
    .filter((p) => p.ballsBowled > 24)
    .sort((a, b) => {
      // Min 4 overs
  ...
  return {
    ...
    awards: {
      ...
      impact: {
        name: impactPlayer?.name || "N/A",
        val: (impactPlayer?.pts || 0).toFixed(0),
      },
      sr: {
        name: bestSR?.name || "N/A",
        val: bestSR ? ((bestSR.runs / bestSR.ballsFaced) * 100).toFixed(2) : 0,
      },
      eco: {
        name: bestEco?.name || "N/A",
        val: bestEco
          ? (bestEco.runsConceded / (bestEco.ballsBowled / 6)).toFixed(2)
          : 0,
      },
    },

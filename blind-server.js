...
io.on("connection", (socket) => {
  ...
  // Start blind auction
  socket.on("start_blind_auction", ({ exchangeEnabled }) => {
    const roomId = getRoomId(socket);
    const r = rooms[roomId];
    ...
    if (r && isAdmin(socket)) {
      ...
      // Build auction queue (reuse from normal auction)
      const queue = buildBlindAuctionQueue();
      r.auctionQueue = queue;
      r.state.isActive = true;
  ...
  socket.on("submit_blind_bid", ({ teamKey, amount }) => {
    ...
    console.log(`Bid received from ${team.name}: ₹${amount}`);

    // Check if all teams have bid
    const activeTeams = r.teams.filter(t => t.isTaken);
    const submittedBids = Object.keys(r.blindAuction.currentBids).length;
    
    if (submittedBids >= activeTeams.length) {
      // All bids submitted, reveal
      setTimeout(() => revealBids(roomId), 1000);
    }
  });
  ...
  socket.on("keep_player", () => {
    ...
    // Clear exchange timer if running
    if (r.blindAuction.exchangeTimerInterval) {
        clearInterval(r.blindAuction.exchangeTimerInterval);
        r.blindAuction.exchangeTimerInterval = null;
    }
  ...
  socket.on("request_exchange", ({ fromTeam, toTeam, player }) => {
    ...
    if (r.blindAuction.winner && r.blindAuction.winner.teamKey === fromTeam) {
      // Send exchange request to target team
      io.to(roomId).emit("exchange_request_received", {
        fromTeam: fromTeam,
        toTeam: toTeam,
        player: player
      });
  ...
  socket.on("accept_exchange", ({ fromTeam }) => {
    ...
    // Find the teams
    const fromTeamObj = r.teams.find(t => t.bidKey === fromTeam);
    const toTeamObj = r.teams.find(t => t.ownerSocketId === socket.id);

    if (fromTeamObj && toTeamObj && r.blindAuction.winner) {
      // Exchange the player
      const winner = r.blindAuction.winner;
      
      // Add player to target team
      toTeamObj.roster.push({
        ...r.blindAuction.currentPlayer,
        price: winner.amount,
        status: "SOLD"
      });
      toTeamObj.totalSpent += winner.amount;
      toTeamObj.totalPlayers += 1;
      toTeamObj.budget -= winner.amount;

      // Update winner team budget (they don't pay but get their budget back)
      fromTeamObj.budget += winner.amount;

      console.log(`✅ ${r.blindAuction.currentPlayer.name} EXCHANGED to ${toTeamObj.name}`);
...
function revealBids(roomId) {
    ...
    console.log('Bids:', bids.map(b => `${b.teamName}: ₹${b.amount}`).join(', '));
    ...
    // Store winner for potential exchange
    if (bids.length > 0 && bids[0].amount > 0) {
        r.blindAuction.winner = bids[0];
        
        // If exchange is enabled, start exchange window
        if (r.blindAuction.exchangeEnabled) {
            setTimeout(() => {
                io.to(roomId).emit("exchange_window_started", {
                    winner: bids[0],
                    duration: r.blindAuction.exchangeTimer
                });
                startExchangeTimer(roomId);
            }, 3000);
        } else {
            // No exchange, finalize immediately
            setTimeout(() => finalizeSale(roomId), 3000);
        }
    } else {
        // No bids, player unsold
        console.log(`${r.blindAuction.currentPlayer.name} - UNSOLD`);
        
        // Do NOT auto-advance. Wait for Admin to click "Next Player".
        // This prevents the "all unsold loop" issue.
    }
}
...
function startExchangeTimer(roomId) {
    ...
    const interval = setInterval(() => {
        ...
        // Emit timer tick to clients
        io.to(roomId).emit("exchange_timer_tick", timeLeft);

        if (timeLeft <= 0) {
            clearInterval(interval);
            // Auto-finalize if time runs out (winner didn't take action)
            console.log('⏰ Exchange timer expired - auto-finalizing sale');
            finalizeSale(roomId);
        }
    }, 1000);
    
    // Store interval so it can be cleared if needed
    r.blindAuction.exchangeTimerInterval = interval;
}
...
function finalizeSale(roomId) {
    ...
    io.to(roomId).emit("exchange_finalized", {
        teams: r.teams,
        message: `${r.blindAuction.currentPlayer.name} sold to ${team.name}`
    });
